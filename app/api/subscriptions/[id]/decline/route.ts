...
// Decline a subscription invitation (LP declining GP's invitation)
export async function POST(
  ...
  try {
    ...
    // Check permission - only subscribers can decline their own subscriptions
    ...
    if (isVercel()) {
      ...
      if (!canManageSubscriptionsForSubscriber(user, subscription.subscriberId)) {
        return NextResponse.json(
          { error: 'You do not have permission to decline subscriptions for this subscriber' },
          { status: 403 }
        )
      }

      // Validate status transition: can only decline if status is "Pending LP Acceptance"
      if (subscription.status !== 'Pending LP Acceptance') {
        return NextResponse.json(
          { error: `Cannot decline subscription with status "${subscription.status}". Only pending subscriptions can be declined.` },
          { status: 400 }
        )
      }
    ...
    return NextResponse.json({ error: 'Failed to decline subscription' }, { status: 500 })
  }
}
